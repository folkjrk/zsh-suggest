...
# ── 0. Locate plugins across Homebrew (Intel/ARM), Linux distros, manual clones ──
...
if command -v fzf >/dev/null; then
  ...
  if fzf --zsh >/dev/null 2>&1; then
    if [[ ! -s $_fzf_cache || $commands[fzf] -nt $_fzf_cache ]]; then
      mkdir -p "${_fzf_cache:h}" && fzf --zsh > "$_fzf_cache"
    fi
    source "$_fzf_cache"
  else
    for _d in /opt/homebrew/opt/fzf/shell /usr/local/opt/fzf/shell /usr/share/fzf /usr/share/doc/fzf/examples; do
      [[ -r $_d/key-bindings.zsh ]] && { source $_d/key-bindings.zsh; source $_d/completion.zsh 2>/dev/null; break }
    done
    unset _d
  fi
  unset _fzf_cache
